...
typedef struct {
	...
	{{end}}
	{{end}}
	char* valid_transitions[MAX_TRANSITIONS];
...
//This function will return valid transitions as guards on input and output variables
void {{$block.Name}}_give_valid_transitions(enforcervars_{{$block.Name}}_t* me);
...
void {{$block.Name}}_give_valid_transitions(enforcervars_{{$block.Name}}_t* me){
	...
	//char *destinationState;
	int i = 0;
	{{range $polI, $pol := $block.Policies}}{{$pfbEnf := getPolicyEnfInfo $block $polI}}
	switch(me->_policy_{{$pol.Name}}_state){
	{{range $sti, $st := $pol.States}}case POLICY_STATE_{{$block.Name}}_{{$pol.Name}}_{{$st.Name}}:
		{{range $tri, $tr := $pfbEnf.OutputPolicy.Transitions}}{{if eq $tr.Source $st.Name}}{{/*
		*/}}{{if not (isViolation $tr.Destination)}}
		me->valid_transitions[i++] = "{{removeBrackets (compileExpression $tr.STGuard)}}";{{end}}
		{{end}}
		{{end}}
		break;
{{end}}{{end}}
