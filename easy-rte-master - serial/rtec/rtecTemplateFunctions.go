...
func removeBrackets(condition string) string {
	// Regular expression to match the conditions
	re := regexp.MustCompile(`\(([^()]+)\)`)

	// Loop to remove brackets
	for re.MatchString(condition) {
	...
	// Check if the last condition has a variable like v, v1, v2, ...
	//lastCondition := strings.TrimSpace(conditions[len(conditions)-1])
	//isMatching := false

	// Check if the last condition matches "v", "v1", "v2", ...
	if len(conditions) > 1 {
		//lastCondition := strings.TrimSpace(parts[len(parts)-1])
		//fmt.Println(lastCondition)
		lastCondition := strings.TrimSpace(conditions[len(conditions)-1])
		if strings.HasPrefix(lastCondition, "v") {
			//fmt.Println("I am inside if")
			// Omit the last condition if it matches 'v', 'v1', 'v2', etc.
			conditions = conditions[:len(conditions)-1]
		}
	}

	// Reassemble the conditions with "&&"
	return strings.Join(conditions, " && ")
}

func isViolation(str1 rtedef.PState) bool {
	return string(str1) == "violation"
}
